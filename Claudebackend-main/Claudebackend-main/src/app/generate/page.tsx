...
function GeneratePageContent() {
  ...
  const [durationInFrames, setDurationInFrames] = useState(initialDuration * fps);
  ...
  // Sync frames when seconds change
  useEffect(() => {
    setDurationInFrames(durationInSeconds * fps);
  ...
  // Auto-trigger generation if prompt came from URL or Plan Mode
  useEffect(() => {
    if (!chatSidebarRef.current || hasAutoStarted) return;

    const isPlanMode = searchParams.get("mode") === "plan";
    
    if (isPlanMode) {
      const planJson = sessionStorage.getItem("saas-production-plan");
      if (planJson) {
        try {
          const plan = JSON.parse(planJson);
          setHasAutoStarted(true);
          setDurationInSeconds(plan.durationInSeconds);
          setPrompt(plan.initialPrompt);
          
          // Combine logo and screenshots for the initial generation
          const allImages: string[] = [];
          if (plan.logo) allImages.push(plan.logo);
          if (plan.screenshots) allImages.push(...plan.screenshots);
          ...
          setTimeout(() => {
            ...
            // Clear once consumed to prevent re-triggering on refresh
            sessionStorage.removeItem("saas-production-plan");
          }, 100);
          return;
        } catch (e) {
    ...
    if (initialPrompt) {
      setHasAutoStarted(true);
      // Check for initial attached images from sessionStorage (Standard chat mode)
  ...
  }, [initialPrompt, hasAutoStarted, searchParams]);
  ...
  return (
    ...
    >
      <div className="flex-1 flex flex-col min-[1000px]:flex-row min-w-0 overflow-hidden">
        ...
        {/* Main content area */}
        <div className="flex-1 flex flex-col min-w-0 pr-8 pb-2 overflow-hidden">
          <TabPanel
            codeContent={
              <CodeEditor
