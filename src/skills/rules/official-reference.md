...
- A Lambda function can be deployed using npx remotion lambda functions deploy
- A site can be deployed using npx remotion lambda sites create. The first argument must refer to the entry point.
...
- A Lambda function can be deployed using deployFunction()
- A site can be deployed using deploySite()
- A video can be rendered using renderMediaOnLambda()
- If a video is rendered, the progress must be polled using getRenderProgress()
